    ...
    return (
        <ToastProvider>
            <AppShell>
                ...
                {/* Barra superior fija — debajo del announcement bar */}
                <motion.header
                    animate={{ top: barVisible ? 38 : 0 }}
                    transition={{ duration: 0.4, ease: [0.04, 0.62, 0.23, 0.98] }}
                    className="fixed left-0 z-50 w-full bg-[#FBCC13] dark:bg-yellow-500 shadow-md text-slate-900 p-4 flex items-center h-[72px] min-h-[72px]"
                >
                    <motion.div
                        className="flex items-center flex-shrink-0"
                        animate={{ scale: isMobileMenuOpen ? 0.95 : 1 }}
                        ...
                        <div className="flex items-center cursor-pointer">
                            {auth?.user ? (
                                ...
                                <div className="flex items-center gap-6">
                                    <Link href={route('orders.guest.lookup')} className="hover:text-[#006CFA] font-medium">Rastrear pedido</Link>
                ...
                <AnimatePresence>
                    {isMobileMenuOpen && (
                        <>
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1, marginTop: barVisible ? 110 : 72 }}
                                exit={{ opacity: 0 }}
                            ...
                            <motion.div
                                ...
                                animate={{
                                    opacity: 1,
                                    y: 0,
                                    top: barVisible ? 110 : 72,
                                    maxHeight: `calc(100vh - ${barVisible ? 110 : 72}px)`
                                }}
                                exit={{ opacity: 0, y: -20 }}
                                transition={{ duration: 0.4, ease: [0.04, 0.62, 0.23, 0.98] }}
                                className="fixed left-0 w-full bg-white dark:bg-slate-950 shadow-2xl overflow-y-auto z-[100] sm:hidden border-t border-slate-100 dark:border-slate-800"
                            >
                                    ...
                                    <div className="p-6 flex flex-col space-y-2">
                                        {auth?.user ? (
                                            ...
                                            <div className="flex flex-col gap-4 py-4">
                                                ...
                                                <Link
                                                    href={route('orders.guest.lookup')}
                                                    className="w-full py-4 rounded-2xl bg-[#FBCC13] text-slate-900 font-black text-sm uppercase tracking-widest text-center shadow-md active:scale-95 transition-all border-2 border-slate-900"
                                                    onClick={handleCloseMobileMenu}
                                                >
                                                    Rastrear Pedido
                                                </Link>
                ...
                <motion.div
                    animate={{ paddingTop: barVisible ? 110 : 72 }}
                    transition={{ duration: 0.4, ease: [0.04, 0.62, 0.23, 0.98] }}
                >
